            ...
            IWorkoutService workoutService) =>
        {
                ...
                IWorkoutService workoutService) =>
            {
                ...
                return success ? Results.Ok(new { message }) : Results.NotFound(new { message });
                        ...
                        // PUT /api/clients/{clientId}/workouts/{workoutId}/days/{dayId}/cardio
                    group.MapPut("/{workoutId:int}/days/{dayId:int}/cardio", async (
                        int clientId,
                        int workoutId,
                        int dayId,
                        CreateCardioDto dto,
                    ...
                    {
                        var coachId = userManager.GetUserId(http.User)!;
                        var (success, message) = await workoutService.UpdateCardioAsync(
                            clientId, workoutId, dayId, dto, coachId);
                        return success
                            ? Results.Ok(new { message })
                            : Results.NotFound(new { message });
